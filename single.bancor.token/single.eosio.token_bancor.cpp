...
#include "single.eosio.token_bancor.hpp"

namespace eosiosystem {
   /*固定为:quote->base*/
   asset exchange_state::convert_to_exchange( asset in ) {

      /*
      real_type R(supply.amount);
      */
      eosio_assert( in.symbol == quote.balance.symbol, "in symbol mismatch" );

      real_type R(base.balance.amount);
      /*real_type C(quote.balance.amount+in.amount);*/
      real_type C(quote.balance.amount);
      real_type F(quote.weight/1000.0);
      ...
      quote.balance.amount += in.amount;
      base.balance.amount += issued;

      return asset( issued, base.balance.symbol );
   }

   asset exchange_state::convert_from_exchange( asset in ) {
      eosio_assert( in.symbol== base.balance.symbol, "unexpected asset symbol input" );

      /*real_type R(base.balance.amount - in.amount);*/
      real_type R(base.balance.amount);
      real_type C(quote.balance.amount);
      real_type F(1000.0/base.weight);
     ...
     // real_type T = C * std::expm1( F * std::log1p(E/R) );
      ...
      return asset( out, quote.balance.symbol );
   }
                  ...
                  )
{
    ...
    if (cw == 1000)
    {
        ...
        eosio_assert( (quantity.amount+pre_releasea.amount+pre_releaseb.amount) == quote_supply.amount, "quantity and quote_supply must be equa" );
    }
    ...
    _market.emplace( _self, [&]( auto& m ) {
        ...
        m.base.balance = quantity + pre_releasea + pre_releaseb;
        m.base.weight = cw;
                      ...
                      string       memo )
{
    ...
    else
    {
        ...
        _market.modify( itr, 0, [&]( auto& es ) {
              token_out = es.convert_from_exchange( quantity );
        });
        in_retire(quantity, "retire by sell");
    }
...
void token::on( const currency::transfer& t, account_name code ) 
{
    ...
    eosio_assert( token_out.symbol == itr->base.balance.symbol, "token_symbol symbol mismatch" );
    ...
    {
        _market.modify( itr, 0, [&]( auto& es ) {
            token_out = es.convert_to_exchange( t.quantity );
        }); 
    }
    ...
    action(
            permission_level{ itr->base.contract, N(active) },
            itr->base.contract, N(issue),
            std::make_tuple(t.from, token_out, std::string("initial.issue for buy"))
    ).send();
...
void eosio::token::apply( account_name contract, account_name act ) 
{
  ...
  switch (act) { EOSIO_API(eosio::token, (create)(issue)(retire)(destroy)(transfer)(setconfig)(close)(regwhitelist)(regadmwhite)(removewhite)); };
}
